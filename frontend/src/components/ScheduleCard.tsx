...
import { formatDateRuLong, isToday } from "../utils/dateUtils";
...
export default function ScheduleCard({ data, onClick }: ScheduleCardProps) {
  const color = isToday(data.date) ? "bg-green-100" : "bg-white";
  ...
  return (
    <div
      onClick={onClick}
      className={`${color} border border-slate-200 rounded-lg p-4 shadow-sm hover:shadow-md hover:border-slate-300 transition-all cursor-pointer`}
    >
