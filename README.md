...
```
### 2. Настройка переменных окружения и запуск
Для настройки переменных окружения рекомендуется использовать один из скриптов в зависимости от ваших целей.

#### Разработка
Если вы хотите запустить проект для разработки, используйте скрипт `dev_setup.sh`, он автоматически заполнит переменныое окружения.
1. Запустите скрипт для автоматического заполнения переменных окружения и **запустите контейнер с БД**:
  ```bash
  $ ./dev_setup.sh
  ```
2. Запустите `backend`:
...
```
Веб-интерфейс будет доступен на дефолтном порту `VITE` по адресу `http://localhost:5173`

#### Эксплуатация
Если вы хотите разместить проект на сервере для дальнейшей **эксплуатации**, используйте скрипт `fast_setup.sh`, он позволит заполнить необходимые переменные окружения, собрать и запустить контейнеры проекта:
```bash
$ ./fast_setup.sh
```
**Или** заполните переменные окружения вручную ***(ниже описаны все переменные окружения, будьте внимательны при заполнении)*** и запустите контейнеры вручную:
